<div class="modal-dialog">
    <div class="modal-content">
        ...
        {!! Form::open(['route' => 'deals.api.save', 'class' => 'form-horizontal ajaxifyForm', 'data-toggle' =>
        'validator']) !!}
                        ...
                        <div class="col-sm-8">
                            <select name="currency" class="form-control select2-option">
                                ...
                                <option value="{{ $cur['code'] }}"
                                    {{ get_option('default_currency') == $cur['code'] ? ' selected' : '' }}>
                                    {{ $cur['native'] }} - {{ $cur['title'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-8">
                            <select class="select2-option form-control" name="source" required>
                                @foreach (App\Entities\Category::select('id', 'name')->whereModule('source')->get() as
                                $key => $source)
                        ...
                        <div class="col-lg-8">
                            <div class="input-group">
                                <input class="datepicker-input form-control" size="16" type="text"
                                    value="{{ datePickerFormat(now()->addDays(get_option('deal_rotting'))) }}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-8">
                            <select class="select2-option form-control" name="organization" required>
                                @foreach (Modules\Clients\Entities\Client::select('id', 'name')->get() as $key =>
                                $client)
                                <option value="{{  $client->id  }}" {{ $company == $client->id ? 'selected' : '' }}>
                                    {{  $client->name  }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-8">
                            <select class="select2-option form-control" name="contact_person" required>
                                ...
                                <option value="{{  $user->id  }}" {{ $contact == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
