...
{
    use BelongsToUser, Taggable, Commentable, Observable, Actionable, Uploadable,
    Eventable, Remindable, Searchable, Todoable, Recurrable, Customizable, HasFactory;
    ...
    {
        return $this->user_id == \Auth::user()->id || \Auth::user()->can('expenses_view_all');
    }
    ...
    {
        $this->before_tax = (new ExpenseCalculator($this))->beforeTax();
        $this->taxed = (new ExpenseCalculator($this))->taxed();
        $this->saveQuietly();
    ...
    public function codeExists($next_number)
    {
        ...
        if ($this->where('code', get_option('expense_prefix') . $next_number)->count() > 0) {
