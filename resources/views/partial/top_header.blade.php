    ...
    <div class="container-fluid">
            ...
            <li class="">
                ...
                <div class="m-t m-l-lg">
                    @if (\Auth::user()->tasksUpcoming() > 0)
                        <a href="{{ route('tasks.upcoming') }}" class="" data-toggle="tooltip"
                            title="@langapp('tasks')" data-placement="bottom">
                            @icon('solid/tasks')
                            <span
                                class="bg-red-500 badge badge-sm up m-l-n-sm display-inline">{{ \Auth::user()->tasksUpcoming() }}</span>
                        </a>
                    @endif
                    @if (\Auth::user()->todoToday() > 0)
                        <a href="{{ route('calendar.todos') }}" class="" data-toggle="tooltip"
                            title="@langapp('todo')" data-placement="bottom">
                            @icon('regular/check-circle')
                            <span
                                class="bg-red-500 badge badge-sm up m-l-n-sm display-inline">{{ \Auth::user()->todoToday() }}</span>
                        </a>
                    @endif

                    @if (Auth::user()->newchats->count())
                        <a href="{{ route('leads.index') }}" class="m-l" data-toggle="tooltip" title="WhatsApp"
                            data-placement="bottom">
                            @icon('solid/comment-alt', 'fa-lg text-success')
                            <span
                                class="badge badge-sm up bg-dracula m-l-n-sm display-inline">{{ Auth::user()->newchats->count() }}</span>
                        </a>
                    @endif

                    <a href="{{ route('calendar.appointments') }}" class="m-l" data-toggle="tooltip"
                        title="@langapp('appointments') " data-placement="bottom">
        ...
        <ul class="nav navbar-nav navbar-right hidden-xs nav-user">
            ...
            @if (count(runningTimers()) > 0)
                <li class="">
                    <a href="{{ route('timetracking.timers') }}" title="@langapp('timers')" data-toggle="ajaxModal"
                        data-toggle="tooltip" data-placement="bottom">
                        <i class="fas fa-sync-alt fa-spin fa-lg {{ themeText() }}"></i>
                        <span
                            class="bg-blue-400 badge badge-sm up m-l-n-sm display-inline">{{ count(runningTimers()) }}</span>
                    </a>
                </li>
            @endif
            ...
            <li class="dropdown">
                ...
                <ul class="dropdown-menu animated fadeInRight">
                    ...
                    @if (config('system.remote_support') && isAdmin())
                        ...
                        <li><a href="{{ route('tell.friend') }}" data-toggle="ajaxModal">@langapp('tell_friend')</a>
                        </li>
                    @endif
                    <li class="divider"></li>
